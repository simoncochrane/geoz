...
type LineIntersector interface {
	// ComputePointIntersection computes the intersection of a point p and the line p1-p2.
	// This computes the HasIntersection boolean test. The actual value of the
	// intersection is equal to p.
	ComputePointIntersection(p, p1, p2 Coordinate)

	// ComputeLineIntersection computes the intersection of the lines p1-p2 and p3-p4.
	ComputeLineIntersection(p1, p2, p3, p4 Coordinate)

	HasIntersection() bool
	NumIntersections() int
	IsProper() bool

	IntersectionAt(intIndex int) Coordinate
	...
	IntersectsPoint(point Coordinate) bool
}
...
func (rli *RobustLineIntersector) IntersectionAt(intIndex int) Coordinate {
...
func (rli *RobustLineIntersector) computeEdgeDistance(p, p0, p1 Coordinate) float64 {
...
func (rli *RobustLineIntersector) IntersectsPoint(point Coordinate) bool {
...
func (rli *RobustLineIntersector) ComputePointIntersection(p, p1, p2 Coordinate) {
	rli.isProper = false

	// do between check first, since it is faster than the orientation test
	if EnvelopeIntersectsPoint(p1, p2, p) {
		if OrientationIndex(p1, p2, p) == 0 && OrientationIndex(p2, p1, p) == 0 {
			rli.isProper = true
			if p.Equals(p1) || p.Equals(p2) {
				rli.isProper = false
			}
			rli.result = LineIntersectionPoint
			return
		}
	}

	rli.result = LineIntersectionNone
}

func (rli *RobustLineIntersector) ComputeLineIntersection(p1, p2, q1, q2 Coordinate) {
...
func (rli *RobustLineIntersector) computeIntersectionResult(p1, p2, q1, q2 Coordinate) LineIntersectionResult {
	rli.isProper = false

	// first try a fast test to see if the envelopes of the lines intersect
	if !EnvelopeIntersects(p1, p2, q1, q2) {
...
func (rli *RobustLineIntersector) computeCollinearIntersection(p1, p2, q1, q2 Coordinate) LineIntersectionResult {
	p1q1p2 := EnvelopeIntersectsPoint(p1, p2, q1)
	p1q2p2 := EnvelopeIntersectsPoint(p1, p2, q2)
	q1p1q2 := EnvelopeIntersectsPoint(q1, q2, p1)
	q1p2q2 := EnvelopeIntersectsPoint(q1, q2, p2)
...
func (rli *RobustLineIntersector) intersection(p1, p2, q1, q2 Coordinate) Coordinate {
...
func (rli *RobustLineIntersector) intersectionWithNormalization(p1, p2, q1, q2 Coordinate) Coordinate {
...
func normalizeToEnvCentre(p1, p2, q1, q2 Coordinate) (n00, n01, n10, n11, normPt Coordinate) {
...
func (rli *RobustLineIntersector) safeHCoordinateIntersection(p1, p2, q1, q2 Coordinate) Coordinate {
...
func hcoordinateIntersection(p1, p2, q1, q2 Coordinate) (Coordinate, error) {
	...
	if math.IsNaN(xInt) || math.IsInf(xInt, 0) || math.IsNaN(yInt) || math.IsInf(yInt, 0) {
		return Coordinate{}, errors.New("Cannot calculate hcoordinate Intersection due to invalid result")
	}
...
func nearestEndpoint(p1, p2, q1, q2 Coordinate) Coordinate {
	nearestPt := p1
	minDist := DistancePointToSegment(p1, q1, q2)

	dist := DistancePointToSegment(p2, q1, q2)
	if dist < minDist {
		minDist = dist
		nearestPt = p2
	}
	dist = DistancePointToSegment(q1, p1, p2)
	if dist < minDist {
		minDist = dist
		nearestPt = q1
	}
	dist = DistancePointToSegment(q2, p1, p2)
	if dist < minDist {
		minDist = dist
		nearestPt = q2
	}
...
func (rli *RobustLineIntersector) isInSegmentEnvelopes(intPt Coordinate) bool {
	env0 := NewEnvelopeFromCoords(rli.inputLines[0][0], rli.inputLines[0][1])
	env1 := NewEnvelopeFromCoords(rli.inputLines[1][0], rli.inputLines[1][1])
